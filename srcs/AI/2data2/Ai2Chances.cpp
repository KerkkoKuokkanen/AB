...
static int SmokeChecker(t_AiCharacter *character, int chance, t_AiMapItem **items)
{
	SDL_Point pos = character->position;
	if (!Ai2PositionIsSmoked(pos, items))
		return (chance);
...
int Ai2GetChance(t_AiCharacter *character, t_AiCharacter *target, t_Ability *ability, t_AiMapItem **items)
{
	...
	baseChance = (ability == NULL) ? baseChance : SmokeChecker(character, baseChance, items);
	if (baseChance > 95)
		baseChance = 95;
	if (baseChance < 5)
		baseChance = 5;
